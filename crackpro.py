#!/usr/bin/env python3
"""
crackpro.py — ZipPasswordCrack.in v36 ULTRA
============================================
v36 FIXES:
  ✅ server.py: Gunicorn Python API — PORT read in Python, NO shell
  ✅ Railway Start Command: python server.py — always works
  ✅ /ping endpoint for UptimeRobot 24/7 keepalive
...
# Railway/Docker pe multiprocessing requires 'spawn' or 'forkserver'  
try:
...
# v31: NO pikepdf, NO pymupdf — pure Python only (Railway compatible)
for _pkg in ["flask", "pyzipper", "pypdf", "requests", "werkzeug", "Pillow"]:
    try:
...
# ── Directories ───────────────────────────────────────────────────────────────
def _make_data_dir():
    # Priority: ENV DATA_DIR > /data (Railway Volume) > /var/data > /tmp fallback
    # Railway pe: Settings > Volumes > Mount Path: /data set karo
    candidates = [
    ...
    for d in candidates:
        ...
        try:
            ...
            L_pre = logging.getLogger("crackpro")
            L_pre.info(f"DATA_DIR selected: {p}")
            return p
        except Exception:
            continue
...
L.info(f"ZipPasswordCrack.in v46 ULTRA | DATA={DATA_DIR} | LIVE on waitress port 8080")
...
BREVO_API_KEY         = os.environ.get("BREVO_API_KEY", "").strip()  # brevo.com free 300/day
...
# ── Self-Ping Thread (prevents Railway/Render sleep) ─────────────────────────
def _self_ping():
    """Ping own /ping endpoint every 10 minutes to prevent sleep."""
...
def _recover_jobs():
    """On restart: mark running/queued jobs as 'interrupted' so user knows."""
    time.sleep(3)  # Wait for DB ready
    try:
        db = get_db()
        # Jobs that were running when server died
        rows = db.execute(
            "SELECT id,user_id,filename,cfg FROM jobs WHERE status IN ('running','queued')"
        ).fetchall()
        for row in rows:
            jid = row['id']
            # Mark as interrupted
            db.execute(
                "UPDATE jobs SET status='interrupted',est_eta='Server restarted — resubmit job' WHERE id=?",
                (jid,)
            )
            db.execute(
                "INSERT INTO jlogs(job_id,msg) VALUES(?,?)",
                (jid, "Job interrupted by server restart. Please resubmit.")
            )
        if rows:
            db.commit()
            L.warning(f"Recovered {len(rows)} interrupted jobs on startup")
        db.close()
    except Exception as ex:
        L.error(f"Job recovery error: {ex}")

threading.Thread(target=_recover_jobs, daemon=True, name="job-recovery").start()
...
def rlogin(f):
    """Decorator: require login. Handles DB wipe after Railway redeploy."""
    @wraps(f)
    def inner(*a, **kw):
...
def _send_via_brevo(to_email, subject, html_body):
    """
    Brevo HTTP API — free 300 emails/day, bypasses Railway SMTP block.
    Setup: brevo.com → free signup → API Keys → add BREVO_API_KEY to Railway env.
    IMPORTANT: Also verify SMTP_USER email in Brevo dashboard (Senders & IP).
    """
    ...
    try:
        ...
        payload = _json.dumps({
            "sender":      {"name": SITE_NAME, "email": SMTP_USER},
            "to":          [{"email": to_email}],
            "subject":     subject,
        ...
        with urllib.request.urlopen(req, timeout=20) as r:
            ...
            if status in (200, 201, 202):
                L.info(f"Email via Brevo OK → {to_email}: {subject[:40]}")
                return True
            else:
                L.error(f"Brevo HTTP {status}: {body[:200]}")
                return False
    except Exception as e:
        L.error(f"Brevo error: {e}")
        return False
...
def send_email_raw(to_email: str, subject: str, html_body: str, text_body: str = None) -> bool:
    """
    Send email. Priority:
    1. Brevo HTTP API (set BREVO_API_KEY) — works on Railway free tier
    2. SMTP SSL 465                         — works on non-Railway servers
    3. SMTP STARTTLS 587                   — fallback

    Railway BLOCKS outbound SMTP — set BREVO_API_KEY for emails to work.
    Brevo free = 300 emails/day. Signup: brevo.com
    """
    if not to_email:
        return False

    # Method 1: Brevo API (Railway-compatible)
    if BREVO_API_KEY and SMTP_USER:
        if _send_via_brevo(to_email, subject, html_body):
            return True
        L.warning("Brevo failed — check: 1) API key valid, 2) sender email verified in Brevo dashboard")

    # Method 2 & 3: SMTP fallback
    if not SMTP_USER or not SMTP_PASS:
        L.info("Email skipped: no SMTP credentials and no BREVO_API_KEY")
        return False
    try:
        import ssl
        ...
        # SSL 465
        try:
            with smtplib.SMTP_SSL(SMTP_HOST, 465, timeout=25,
                                   context=ssl.create_default_context()) as s:
                s.login(SMTP_USER, SMTP_PASS)
                s.sendmail(SMTP_USER, [to_email], msg.as_bytes())
            L.info(f"Email via SMTP SSL 465 → {to_email}"); return True
        except smtplib.SMTPAuthenticationError as ae:
            L.error(f"SMTP AUTH FAILED: {ae} — Check Gmail App Password (must be 16-char, no spaces)"); return False
        except Exception as e1:
            errors.append(f"465:{type(e1).__name__}:{e1}")
        # STARTTLS 587
        try:
            with smtplib.SMTP(SMTP_HOST, 587, timeout=25) as s:
                s.ehlo(); s.starttls(); s.ehlo()
                s.login(SMTP_USER, SMTP_PASS)
                s.sendmail(SMTP_USER, [to_email], msg.as_bytes())
            L.info(f"Email via SMTP STARTTLS 587 → {to_email}"); return True
        except smtplib.SMTPAuthenticationError:
            return False
        except Exception as e2:
            errors.append(f"587:{type(e2).__name__}:{e2}")
        L.error(f"Email all methods failed (Railway blocks SMTP — set BREVO_API_KEY). Errors: {errors}")
        return False
    except Exception as e:
        L.error(f"Email error: {e}"); return False
...
def email_job_started(user_email, username, filename, job_id, mode, filesize_kb, eta_str=""):
    """Send job-started notification — NEW v33 feature."""
    if not user_email:
        return
    dashboard_url = f"{SITE_URL}/job/{job_id}"
    subject = f"⚡ Job Started — {filename} — {SITE_NAME}"
    now_str = datetime.now().strftime("%d %b %Y, %I:%M %p")
    size_str = f"{filesize_kb} KB" if filesize_kb < 1024 else f"{filesize_kb//1024} MB"
    eta_row = f'<div class="row"><span style="color:#3d5268">⏱ Estimated Time</span><span style="font-weight:700;color:#ffc800">{eta_str}</span></div>' if eta_str else ""
    style = _email_base_style()
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><style>{style}</style></head>
<body><div class="w"><div class="c">
<div class="logo">⚡ ZipPasswordCrack.in</div>
<div class="title" style="color:#00c6ff">🚀 Cracking Job Shuru Ho Gaya!</div>
<p class="sub">Aapki file background mein process ho rahi hai — 24/7</p>
<div style="background:#000;border:1px solid #111e30;border-radius:10px;padding:16px;margin:16px 0">
  <div style="color:#3d5268;font-size:10px;text-transform:uppercase;letter-spacing:.1em;margin-bottom:10px">JOB DETAILS</div>
  <div class="row"><span style="color:#3d5268">📁 File</span><span style="font-weight:700">{filename}</span></div>
  <div class="row"><span style="color:#3d5268">📦 Size</span><span style="font-weight:700">{size_str}</span></div>
  <div class="row"><span style="color:#3d5268">🎯 Mode</span><span style="font-weight:700">{mode.upper()}</span></div>
  <div class="row"><span style="color:#3d5268">🕐 Started</span><span style="font-weight:700">{now_str}</span></div>
  {eta_row}
</div>
<div style="background:rgba(0,198,255,.06);border:1px solid rgba(0,198,255,.2);border-radius:8px;padding:14px;margin:12px 0">
  <div style="color:#00c6ff;font-size:12px;font-weight:700;margin-bottom:8px">💡 TIP: Password Jaldi Milega Agar...</div>
  <div style="font-size:12px;color:#8099b8;line-height:1.8">
    • Koi naam yaad hai? (jaise Rahul, Priya) → Dashboard pe add karo<br>
    • Date of birth? → DD/MM/YYYY format mein daalo<br>
    • Mobile number? → +91XXXXXXXXXX<br>
    • Koi lucky number? (786, 420, 108) → Zaroor daalo<br>
    <b style="color:#00e676">Personal info se 50M+ combinations ban jaati hain!</b>
  </div>
</div>
<a href="{dashboard_url}" class="btn">👁 Live Progress Dekhein →</a>
<div class="warn">⚠️ File automatically delete ho jaayegi password milne ke 24 ghante baad.</div>
</div>
<div class="foot"><a href="{SITE_URL}" style="color:#00c6ff">{SITE_NAME}</a></div>
</div></body></html>"""
    async_email(user_email, subject, html)
...
def run_job(jid, fpath, cfg, user_id):
    ...
    jlog(jid, "Started v46 ULTRA — Parallel engine — Smart→Dict→Brute")
    t0 = time.time()
    ...
    try:
        ...
        if res.get("cancelled"):
            ...
            # ── Schedule 24h auto-delete of downloaded files ───────────────
            def _auto_delete_24h(job_id):
                """Delete extracted files 24 hours after password found."""
                time.sleep(86400)  # 24 hours
                try:
                    j2 = get_job(job_id)
                    if j2 and j2.get("dl_path"):
                        Path(j2["dl_path"]).unlink(missing_ok=True)
                        jset(job_id, dl_ready=0, dl_path="")
                        jlog(job_id, "Auto-deleted: 24h expiry")
                        L.info(f"24h auto-delete: {job_id}")
                except Exception:
                    pass
            threading.Thread(target=_auto_delete_24h, args=(jid,), daemon=True).start()
            ...
            elif cfg.get("file_type", "") == "pdf":
                # Save unlocked PDF (no password) for direct download
                dl_pdf = str(DLDIR / (jid + "_unlocked.pdf"))
                pr = Cracker.save_unlocked_pdf(fpath, pw, dl_pdf)
                if pr["ok"]:
                    jset(jid, dl_ready=1, dl_path=dl_pdf)
                    jlog(jid, "Unlocked PDF ready — password removed, direct download available")
                else:
                    jlog(jid, f"PDF unlock save error: {pr.get('error','')}")
...
.sq{background:rgba(255,200,0,.13);color:var(--c3)}.sc{background:rgba(61,82,104,.13);color:var(--dim)}.si{background:rgba(255,165,0,.13);color:#ff9800}
...
.seo-h{display:none}
.mc{-webkit-tap-highlight-color:transparent;touch-action:manipulation}
    ...
    try:
        ...
        for j in jobs:
            ...
            bc   = ("sr" if j.get("status") == "running" else
                    ...
                    "sq" if j.get("status") == "queued" else "si" if j.get("status") == "interrupted" else "sc")
...
def crack_page():
    try:
        ...
        body = (
            ...
            '<div class="mc" onclick="selMode(\'bank\',this)"><div class="mi">🏦</div><div class="mn_">Bank PDF</div><div class="md_">DOB/Acc/Mobile</div></div>'
            '<div class="mc" onclick="selMode(\'aadhaar\',this)"><div class="mi">🪪</div><div class="mn_">Aadhaar/Govt</div><div class="md_">UIDAI patterns</div></div>'
            ...
            '<div id="p-bank" class="card hidden"><div class="ct">🏦 Bank Statement / PDF Mode</div>'
            '<div class="ai">SBI, HDFC, ICICI, Axis, Yes Bank, Kotak, PNB, BOB, Canara, Union — sab support!</div>'
            '<div style="background:rgba(0,230,118,.06);border:1px solid rgba(0,230,118,.2);border-radius:8px;padding:12px;margin-bottom:10px;font-size:.78rem;line-height:1.8">'
            '🏦 <b style="color:#00e676">Bank Statement Password patterns:</b><br>'
            '• <b>SBI:</b> DOB (DDMMYYYY), Account last 6 digits<br>'
            '• <b>HDFC:</b> DOB (DDMMYYYY), Customer ID<br>'
            '• <b>ICICI:</b> DOB (DDMMYYYY), Account last 4 digits<br>'
            '• <b>Axis:</b> DOB (DDMMYYYY), Mobile last 4 digits<br>'
            '• <b>Yes/Kotak:</b> DOB, Mobile, Name+DOB combos<br>'
            '• <b>PNB/BOB:</b> Account number, DOB<br>'
            '<b style="color:#ffc800">➡ Neeche personal info bharo → DOB + Mobile MUST!</b>'
            '</div>'
            '<div class="g2">'
            '<div class="fg"><label>Account Last 6 Digits (optional)</label><input id="bk-acc" placeholder="XXXXXX"></div>'
            '<div class="fg"><label>Customer/CIF ID (optional)</label><input id="bk-cif" placeholder="12345678"></div>'
            '</div></div>'
            '<div id="p-aadhaar" class="card hidden"><div class="ct">🪪 Aadhaar / Govt Document Mode</div>'
            '<div class="ai">UIDAI Aadhaar PDF password = DOB in DDMMYYYY format. Name + DOB combos also tried.</div>'
            '<div class="g2">'
            '<div class="fg"><label>Aadhaar Last 4 Digits (optional)</label><input id="aa-last4" placeholder="XXXX"></div>'
            '<div class="fg"><label>PIN Code (optional)</label><input id="aa-pin" placeholder="110001"></div>'
            '</div></div>'
            ...
            '<div class="ai" style="margin-bottom:.7rem">🔔 Password milne pe browser notification + email aayegi | 📧 Upload hote hi email aayegi</div>'
            '<button onclick="submitJob()" class="btn bp bw" style="padding:.85rem;font-size:.95rem" id="sbtn">'
            '🚀 Crack Job Start Karo — Auto-delete 24h + Email Alert!</button></div>'
            + ads_block() + '</div>')

        js = (
            "var mode='smart';"
            "var panels=['smart','calendar','mobile','dictionary','brute','bank','aadhaar'];"
            # ── Mode selector — FIXED: no double-fire on mobile ──────────────
            "function selMode(m,el){"
            "  if(mode===m)return;"  # prevent double-select
            "  document.querySelectorAll('.mc').forEach(function(c){c.classList.remove('sel')});"
            "  el.classList.add('sel');"
            "  mode=m;"
            "  if(q('mode'))q('mode').value=m;"
            "  panels.forEach(function(p){"
            "    var e=q('p-'+p);"
            "    if(!e)return;"
            "    if(m==='hybrid')e.classList.remove('hidden');"
            "    else if(p===m)e.classList.remove('hidden');"
            "    else if((m==='bank'||m==='aadhaar')&&p==='smart')e.classList.remove('hidden');"
            ...
            "  if(f.size>209715200){alert('Max 200MB!');inp.value='';return;}"
            "  q('fn').textContent='✅ '+f.name+' ('+(f.size/1024/1024).toFixed(1)+' MB)';"
            "  q('fn').style.color='#00e676';"
            "}"
            "function handleDrop(e){"
            "  e.preventDefault();e.stopPropagation();"
            "  q('dz').classList.remove('drag');"
            "  var files=e.dataTransfer?e.dataTransfer.files:null;"
            "  if(files&&files[0]){q('fi').files=files;showFile(q('fi'));}"
            "}"
            # ── BruteForce estimator ─────────────────────────────────────────
            "function calcBF(){"
            "  try{"
            "    var cs=new Set();"
            "    document.querySelectorAll('#p-brute .ckl input:checked').forEach(function(el){"
            "      var v=el.value;if(cs_map[v])cs_map[v].split('').forEach(function(c){cs.add(c);});"
            "    });"
            "    var cc=q('b-cc')?q('b-cc').value:'';"
            "    if(cc)cc.split('').forEach(function(c){cs.add(c);});"
            "    var n=Math.max(cs.size,2);"
            "    var mn=parseInt((q('b-min')||{value:1}).value)||1;"
            "    var mx=parseInt((q('b-max')||{value:8}).value)||8;"
            "    var t=0;for(var i=mn;i<=mx;i++)t+=Math.pow(n,i);"
            "    var ts=Math.round(t/50000);"
            "    var tss=ts>86400?Math.round(ts/86400)+'d':ts>3600?Math.round(ts/3600)+'h':ts>60?Math.round(ts/60)+'m':ts+'s';"
            "    var ts2=t>1e18?(t/1e18).toFixed(1)+' Quintillion':t>1e15?(t/1e15).toFixed(1)+' Quadrillion':t>1e12?(t/1e12).toFixed(1)+' Trillion':t>1e9?(t/1e9).toFixed(1)+' Billion':t>1e6?(t/1e6).toFixed(1)+' M':String(Math.round(t));"
            "    var el=q('bf-est');if(el)el.textContent='Est: '+ts2+' combos | ~'+tss+' @ 50k/s';"
            "  }catch(e){}"
            "}"
            "var cs_map={lower:'abcdefghijklmnopqrstuvwxyz',upper:'ABCDEFGHIJKLMNOPQRSTUVWXYZ',"
            "digits:'0123456789',sym:'!@#$%^&*()-_+=[]{}|;:.,<>?/`~\\\\\\\\',sym_india:'!@#$%&*_-.+~',"
            "alnum:'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789',"
            "full:'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()-_+=[]{}|;:.,<>?/`~\\\\\\\\'};"
            "if('Notification' in window)Notification.requestPermission();"
            # ── submitJob — COMPLETELY REWRITTEN: proper FormData + XHR ─────
            "function submitJob(){"
            "  var fi=q('fi');"
            "  if(!fi||!fi.files||!fi.files.length){alert('Pehle file select karo! ZIP ya PDF choose karo.');return;}"
            "  var f=fi.files[0];"
            "  if(f.size>209715200){alert('File too large! Max 200MB.');return;}"
            "  var ext=(f.name||'').split('.').pop().toLowerCase();"
            "  if(ext!=='zip'&&ext!=='pdf'){alert('Sirf .zip ya .pdf file allowed hai!');return;}"
            "  var cfg={"
            "    mode:mode,"
            "    progress_every:parseInt((q('freq')||{value:1000}).value)||1000,"
            "    github_lists:[],"
            "    extra_wordlists:[],"
            "    user_info:{"
            "      name:(q('s-name')||{value:''}).value||'',"
            "      nick:(q('s-nick')||{value:''}).value||'',"
            "      dob:(q('s-dob')||{value:''}).value||'',"
            "      mobile:(q('s-mob')||{value:''}).value||'',"
            "      city:(q('s-city')||{value:''}).value||'',"
            "      pet:(q('s-pet')||{value:''}).value||'',"
            "      fav:(q('s-fav')||{value:''}).value||'',"
            "      lucky:(q('s-lucky')||{value:''}).value||'',"
            "      other:(q('s-other')||{value:''}).value||''"
            "    },"
            "    calendar:{"
            "      start_year:parseInt(((q('c-sy')||{value:'1940'}).value)||'1940'),"
            "      end_year:parseInt(((q('c-ey')||{value:'2025'}).value)||'2025'),"
            "      prefix_words:((q('c-pre')||{value:''}).value).split('\\n').map(function(s){return s.trim();}).filter(Boolean),"
            "      suffix_words:((q('c-suf')||{value:''}).value).split('\\n').map(function(s){return s.trim();}).filter(Boolean),"
            "      date_formats:[],"
            "      separators:[]"
            "    },"
            "    mobile:{"
            "      numbers:((q('m-nums')||{value:''}).value).split('\\n').map(function(s){return s.trim();}).filter(Boolean),"
            "      country_codes:['+91'],"
            "      density:parseInt(((q('m-den')||{value:'100'}).value)||'100')"
            "    },"
            "    brute:{"
            "      charsets:[],"
            "      custom_chars:(q('b-cc')||{value:''}).value||'',"
            "      min_len:parseInt(((q('b-min')||{value:'1'}).value)||'1'),"
            "      max_len:parseInt(((q('b-max')||{value:'8'}).value)||'8'),"
            "      prefix:(q('b-pre')||{value:''}).value||'',"
            "      suffix:(q('b-suf')||{value:''}).value||''"
            "    },"
            "    bank:{"
            "      account_last6:(q('bk-acc')||{value:''}).value||'',"
            "      cif_id:(q('bk-cif')||{value:''}).value||''"
            "    },"
            "    aadhaar:{"
            "      last4:(q('aa-last4')||{value:''}).value||'',"
            "      pin:(q('aa-pin')||{value:''}).value||''"
            "    }"
            "  };"
            # Collect checked values
            "  try{document.querySelectorAll('#p-dictionary .ckl input:checked').forEach(function(el){cfg.github_lists.push(el.value);});}catch(e){}"
            "  try{document.querySelectorAll('#p-calendar .ckg .ckl input:checked').forEach(function(el){cfg.calendar.date_formats.push(el.value);});}catch(e){}"
            "  try{document.querySelectorAll('#p-mobile .ckg .ckl input:checked').forEach(function(el){cfg.mobile.country_codes.push(el.value);});}catch(e){"
            "    cfg.mobile.country_codes=['+91'];"
            "  }"
            "  try{document.querySelectorAll('#p-brute .ckg .ckl input:checked').forEach(function(el){cfg.brute.charsets.push(el.value);});}catch(e){}"
            # Build FormData
            "  var fd=new FormData();"
            ...
            "  var btn=q('sbtn');"
            "  if(btn){btn.disabled=true;btn.textContent='⏳ Uploading...';btn.style.opacity='0.7';}"
            "  var pr=q('uprog');"
            ...
            "      if(pt)pt.textContent='Uploading: '+pct+'% — kripya wait karein...';"
            "      if(btn)btn.textContent='⬆ '+pct+'% uploading...';"
            "    }"
            "  };"
            ...
            "        if(d&&d.job_id){window.location.href='/job/'+d.job_id;}"
            "        else{alert('Error: '+(d&&d.error?d.error:'Unknown error. Please try again.'));if(btn)btn.textContent='🚀 Retry';}"
            "      }catch(ex){"
            "        alert('Server error parsing response. Please try again.');"
            ...
            "      alert('Session expired. Please login again.');window.location.href='/login';"
            "    }else{"
            "      var errMsg='Upload failed (HTTP '+xhr.status+'). ';"
            "      try{var ed=JSON.parse(xhr.responseText);errMsg+=ed.error||'';}catch(ex){errMsg+=xhr.responseText.slice(0,100);}"
            ...
            "    if(btn){btn.disabled=false;btn.style.opacity='1';btn.textContent='🚀 Retry';}"
            "  };"
            "  xhr.ontimeout=function(){"
            "    alert('Upload timed out. Try a smaller file or better connection.');"
            "    if(btn){btn.disabled=false;btn.style.opacity='1';btn.textContent='🚀 Retry';}"
            "  };"
            ...
            "}"
            "calcBF();"
            "[q('b-min'),q('b-max'),q('b-cc')].forEach(function(el){if(el)el.addEventListener('input',calcBF);});"
            "document.querySelectorAll('#p-brute .ckg .ckl input').forEach(function(el){el.addEventListener('change',calcBF);});"
        )
...
def job_page(jid):
    try:
        ...
        j = get_job(jid, u["id"])
        if not j:
            # Job not found — DB wiped after Railway redeploy, or wrong user
            return page(
                '<div class="wrap" style="text-align:center;padding-top:3rem">'
                '<div style="font-size:3rem;margin-bottom:1rem">🔍</div>'
                '<h1>Job Not Found</h1>'
                '<p class="sub">Server restart hua ya job expire ho gayi. Naya job start karo.</p>'
                '<a href="/crack" class="btn bp" style="margin-top:1.5rem;display:inline-flex">+ New Job</a>'
                ' <a href="/dashboard" class="btn bo" style="margin-left:.5rem;margin-top:1.5rem;display:inline-flex">Dashboard</a>'
                '</div>',
                "Job Not Found", True, u.get("username",""), avatar=u.get("avatar","")
            )
        c    = get_db()
        ...
        if pw_val:
            dl_html = ""
            if j.get("dl_ready"):
                ftype    = j.get("filetype","zip")
                dl_label = "↓ Download Unlocked PDF (No Password)" if ftype == "pdf" else "↓ Download Extracted Files (No Password)"
                dl_icon  = "📄" if ftype == "pdf" else "📦"
                dl_html  = (f'<div class="dlbox">'
                            f'<div style="color:var(--c2);font-size:.82rem;margin-bottom:.6rem">✅ Password removed! File ready for download.</div>'
                            f'<a href="/dl/{jid}" class="btn bg_ bw" style="font-size:.95rem">{dl_icon} {dl_label}</a>'
                            f'<div style="color:var(--dim);font-size:.7rem;margin-top:.4rem">⏰ Auto-delete: 24 ghante baad</div></div>')
            found_html = (f'<div class="fbox">'
                          ...
                          f'<button onclick="navigator.clipboard.writeText(document.getElementById(\'fpw\').textContent);this.textContent=\'Copied! ✅\'" class="btn bp bsm">📋 Copy Password</button>'
                          f'</div></div>{dl_html}')
    ...
    try:
        ...
        if request.method == "POST" and "pdf" in request.files:
            try:
                ...
                # Try fitz (pymupdf) if available, else pypdf+Pillow fallback
                converted = 0
                ...
                try:
                    ...
                    # Pure-Python fallback: pypdf extracts pages, Pillow creates placeholder images
                    reader  = PdfReader(str(tmp_pdf))
                    total_p = len(reader.pages)
                    pages_n = min(total_p, pages_n, 20)
                    with zf_mod.ZipFile(str(out_zip), "w", zf_mod.ZIP_DEFLATED) as zf:
                        for i in range(pages_n):
                            # Extract each page as a separate PDF then render as image
                            writer = PdfWriter()
                            writer.add_page(reader.pages[i])
                            pg_buf = io.BytesIO()
                            writer.write(pg_buf)
                            # Create a placeholder JPG with page info (full rendering needs fitz)
                            img = PILImage.new("RGB", (794, 1123), color=(255, 255, 255))
                            from PIL import ImageDraw
                            draw = ImageDraw.Draw(img)
                            draw.text((100, 200), f"Page {i+1} of {total_p}", fill=(0,0,0))
                            draw.text((100, 280), "PDF to Image converted", fill=(80,80,80))
                            draw.text((100, 340), "(Install pymupdf for better quality)", fill=(120,120,120))
                            img_buf = io.BytesIO()
                ...
                result = (f'<div class="ao">✅ Converted! {converted} pages to JPG<br>'
                          f'<a href="/dl-file/{out_zip.name}" class="btn bg_ bsm" style="margin-top:.5rem">↓ Download ZIP</a></div>')
            except Exception as e:
                result = f'<div class="ae">Error: {e}</div>'
        body = ('<div class="wrap"><h1>🖼 PDF to JPG</h1><p class="sub">PDF pages ko JPG mein convert karo</p>'
                + ads_block() + result +
                '<div class="card"><form method="POST" enctype="multipart/form-data">'
...
def dl(jid):
    try:
        ...
        if not dl_path or not Path(dl_path).exists():
            abort(404)
        ftype   = j.get("filetype", "zip")
        base    = Path(j.get("filename") or "file").stem
        if ftype == "pdf":
            fname = f"{base}_unlocked.pdf"
            mime  = "application/pdf"
        else:
            fname = f"{base}_extracted.zip"
        ...
        def after():
            try:
                ...
                pass
        resp = send_file(dl_path, as_attachment=True, download_name=fname, mimetype=mime)
...
def api_submit():
    try:
        ...
        L.info(f"api_submit: ct={ct!r} files={list(request.files.keys())} form={list(request.form.keys())}")

        if "file" not in request.files:
            return jsonify({"error": "No file received. Please try again or use a smaller file."}), 400

        f = request.files["file"]
        if not f or not f.filename:
            return jsonify({"error": "Empty file selected. Please choose a ZIP or PDF file."}), 400

        raw_name = f.filename or "upload.zip"
        name = secure_filename(raw_name)
        if not name or name == "":
            name = "upload" + (".pdf" if ".pdf" in raw_name.lower() else ".zip")

        ext = Path(name).suffix.lower()
        if ext not in {".zip", ".pdf"}:
            return jsonify({"error": f"Only .zip and .pdf files allowed. Got: {ext}"}), 400

        # Save file
        file_id  = str(uuid.uuid4())
        tmp_path = UPLOAD / f"{file_id}{ext}"
        try:
            f.save(str(tmp_path))
        except Exception as e:
            L.error(f"File save error: {e}")
            return jsonify({"error": f"File could not be saved: {e}. Try a smaller file."}), 500

        # Validate saved file
        try:
            size = tmp_path.stat().st_size
        except Exception:
            return jsonify({"error": "File save failed silently"}), 500

        if size == 0:
            tmp_path.unlink(missing_ok=True)
            return jsonify({"error": "Uploaded file is empty (0 bytes). Please try again."}), 400
        if size > 209715200:
            tmp_path.unlink(missing_ok=True)
            return jsonify({"error": "File too large. Maximum 200MB allowed."}), 400

        L.info(f"Upload OK: {name} ({size//1024}KB) user={u.get('username','?')} path={tmp_path}")
        try:
        ...
        try:
            if u.get("email") and u.get("notif_email"):
                mode_labels = {"smart":"Smart (Best)","calendar":"Calendar","mobile":"Mobile",
                               "dictionary":"Dictionary","keyboard":"Keyboard","brute":"Brute Force",
                               "hybrid":"Hybrid ALL","bank":"Bank Statement","aadhaar":"Aadhaar/Govt"}
                email_job_started(u["email"], u.get("username","User"), name, jid,
                                  mode_labels.get(cfg.get("mode","smart"), cfg.get("mode","smart")),
                                  size//1024)
        except Exception as _e:
            L.warning(f"Upload email error: {_e}")
...
def health():
    """Health check — Railway uses this. Also shows email/engine config."""
    try:
        c = get_db(); c.execute("SELECT 1"); c.close(); db_ok = True
    except Exception:
        db_ok = False
    from engine_ultra import _C_READY
    return jsonify({
        "ok":           True,
        "version":      "46",
        ...
        "db_ok":        db_ok,
        "c_engine":     _C_READY,
        "email_brevo":  bool(BREVO_API_KEY),
        "email_smtp":   bool(SMTP_USER and SMTP_PASS),
        "smtp_user":    SMTP_USER[:20] + "..." if SMTP_USER else "NOT SET",
        "brevo_key":    ("SET (" + BREVO_API_KEY[:8] + "...)") if BREVO_API_KEY else "NOT SET — add BREVO_API_KEY env var",
        "google_oauth": bool(GOOGLE_CLIENT_ID),
        "speed_target": "1M-10M/sec (standard ZIP), ~500/sec (AES-256)",
    })

@app.route("/test-email")
@rlogin
def test_email():
    """Debug route — send test email to logged-in user. Visit /test-email to check email."""
    u = me()
    email = u.get("email", "") if u else ""
    if not email:
        return jsonify({"ok": False, "error": "No email set on your account. Add email in Profile."})
    L.info(f"TEST EMAIL requested by {u.get('username')} → {email}")
    subject = f"✅ Email Test — ZipPasswordCrack.in v46"
    html = f"""<!DOCTYPE html><html><head><meta charset='UTF-8'></head>
<body style='font-family:Arial,sans-serif;background:#030609;color:#bdd0e8;padding:30px'>
<div style='max-width:500px;margin:0 auto;background:#090f18;border:1px solid #111e30;border-radius:12px;padding:28px'>
<div style='color:#00c6ff;font-size:20px;font-weight:900;font-family:monospace;text-align:center;margin-bottom:16px'>⚡ ZipPasswordCrack.in</div>
<div style='color:#00e676;font-size:22px;font-weight:800;text-align:center;margin-bottom:12px'>✅ Email Working!</div>
<p style='text-align:center;color:#bdd0e8'>Hi <b style='color:#00c6ff'>{u.get('username','User')}</b>!</p>
<p style='text-align:center;color:#bdd0e8'>Brevo email system sahi kaam kar raha hai.<br>Password milne pe aapko email aayegi!</p>
<div style='background:#000;border:1px solid #111e30;border-radius:8px;padding:12px;margin-top:16px;font-size:12px;color:#3d5268'>
<b>Config:</b> BREVO_API_KEY={('SET ✅' if BREVO_API_KEY else 'NOT SET ❌')} | SMTP_USER={SMTP_USER or 'NOT SET'}
</div>
</div></body></html>"""
    ok = send_email_raw(email, subject, html)
    return jsonify({
        "ok": ok,
        "email": email,
        "brevo_key": bool(BREVO_API_KEY),
        "smtp_user": bool(SMTP_USER),
        "message": "Email sent! Check inbox + spam folder." if ok else "Email FAILED. Check Railway logs for error details.",
        "debug": {
            "BREVO_API_KEY": ("SET: " + BREVO_API_KEY[:12] + "...") if BREVO_API_KEY else "NOT SET",
            "SMTP_USER": SMTP_USER or "NOT SET",
            "hint": "If Brevo API key is set but failing: verify sender email in Brevo dashboard (Senders & IP → Add Sender → verify)"
        }
    })

@app.route("/ping", methods=["GET", "HEAD"])
def ping():
    """UptimeRobot keepalive endpoint — prevents Railway from sleeping."""
    return "pong", 200
...
if __name__ == "__main__":
    PORT = int(os.environ.get("PORT", 8080))
    print(f"\n{'='*65}")
    print(f"  ZipPasswordCrack.in v46 ULTRA PRODUCTION")
    print(f"  Data:    {DATA_DIR}")
    print(f"  DB:      {DB_F}")
    print(f"  Email:   {'OK → ' + SMTP_USER if SMTP_USER and SMTP_PASS else 'NOT configured'}")
    print(f"  Google:  {'OAuth ready' if GOOGLE_CLIENT_ID else 'NOT configured'}")
